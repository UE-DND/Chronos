<script lang="ts">
	...
	import { getMarketplaceService, builtinPlugins } from '$lib/services/app-engine';
	...
	import { DeleteFill, Refresh, CheckCircleFill } from '$lib/icons';
...
<div class="mx-auto flex w-full max-w-lg flex-col gap-4 px-4 pt-1 pb-4 text-on-surface">
	<SegmentedControl
		segments={[
			{
				value: 'installed',
	...
	{#if activeTab === 'installed'}
		<div class="flex flex-col gap-3">
			<h2 class="m3-title-small px-1 text-on-surface-variant">内置官方插件</h2>
						...
						<div class="flex items-start justify-between gap-3">
							<div class="min-w-0 flex-1">
								<div class="flex flex-wrap items-center gap-2">
									<h3 class="m3-title-medium text-on-surface">{name}</h3>
									<span
										class="m3-label-small rounded-full bg-secondary-container px-2 py-0.5 text-on-secondary-container"
									>
										内置
									</span>
								</div>
								{#if desc}
									<p class="m3-body-medium mt-1.5 text-on-surface-variant">{desc}</p>
								{/if}
							</div>
			...
			<h2 class="m3-title-small mt-1 px-1 text-on-surface-variant">第三方已安装插件</h2>

			{#if installedRecords.length === 0}
				<div
					class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-outline/30 px-4 py-8 text-center text-on-surface-variant"
				>
				...
				{#each installedRecords as record (record.manifest.id)}
					...
					<Card variant="outlined">
						<div class="flex flex-col gap-2.5 p-4">
							...
							{#if (record.manifest.permissions || record.manifest.capabilities)?.length}
								<div class="flex flex-wrap gap-1.5">
									{#each record.manifest.permissions || record.manifest.capabilities || [] as perm (perm)}
										<span
											class="m3-label-small rounded-full bg-surface-container-high px-2 py-0.5 text-on-surface-variant"
										>
							...
							<div
								class="flex items-center justify-end gap-2 border-t border-outline-variant/40 pt-2.5"
							>
	...
	{:else}
		<div class="flex flex-col gap-3">
			<div class="flex items-center justify-between px-1">
				<h2 class="m3-title-small text-on-surface-variant">社区与在线插件</h2>
				<Button
					variant="text"
					class="text-xs"
			...
			{:else if availablePlugins.length === 0}
				<div
					class="flex flex-col items-center justify-center rounded-2xl border border-dashed border-outline/30 px-4 py-12 text-center text-on-surface-variant"
				>
					...
					<Card variant="outlined">
						<div class="flex flex-col gap-2.5 p-4">
							<div class="flex items-start justify-between gap-3">
								...
								{#if installed}
									<div class="flex shrink-0 items-center gap-1 text-xs font-medium text-primary">
