...
export const haptic = {
	/** 轻微反馈：Tab 切换、按钮/开关点击、Radio 勾选 (10ms) */
	light(): boolean {
		return triggerVibrate(10);
	},

	/** 中度反馈：周数滑动吸附、分段切换 (20ms) */
	medium(): boolean {
		return triggerVibrate(20);
	},

	/** 重度/确认反馈：长按课程卡片触发、拖拽开始 (35ms) */
	heavy(): boolean {
		return triggerVibrate(35);
	},

	/** 成功反馈：保存成功、导入成功 ([15, 60, 20]) */
	success(): boolean {
		return triggerVibrate([15, 60, 20]);
	},

	/** 警告/危险反馈：删除课程、重置设置 ([30, 50, 30]) */
	warning(): boolean {
		return triggerVibrate([30, 50, 30]);
	},
