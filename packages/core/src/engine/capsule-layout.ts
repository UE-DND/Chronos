...
/** Hide campus name when effective column width is below this. */
 ...
 * UI renders the result; DOM fitting stays across the seam.
 */
...
export const ALL_CORNERS_ROUNDED: CapsuleCorners = {
	topLeft: true,
	topRight: true,
	bottomLeft: true,
	bottomRight: true
...
export const ALL_CORNERS_SQUARE: CapsuleCorners = {
	topLeft: false,
	topRight: false,
	bottomLeft: false,
	bottomRight: false
 ...
 * `compact` subtracts one fixed tier from each size.
 */
...
export function parseLocationParts(location: string): LocationParts {
	...
	// Building name + room code glued or spaced (e.g. ScienceBuildingA101).
