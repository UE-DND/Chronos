...
import { mergeWeekPayloads, resolveWeeksToFetch, type OnlineSchedulePayload } from '../week-merge';
import type { CqutCampusId } from '../campus-period-times';
...
export async function fetchCqutSchedule(
	...
	try {
		const loginResult = await loginCas(session, input.account, password, signal);
		if (!loginResult.ok) return loginResult;

		const campusTimesResult = await fetchCampusTimesForImport(session, signal);
		if (!campusTimesResult.ok) return campusTimesResult;

		const timetableResult = await fetchTimetable(session, input.account, password, input, signal);
...
async function fetchTimetable(
	...
	const weekResults = await mapWithConcurrency(
		...
		WEEK_FETCH_CONCURRENCY,
		async (targetWeek) =>
			fetchWeekEvents(session, account, targetWeek, targetYearTerm, password, true, signal)
	);
...
async function fetchWeekEvents(
	...
	if (looksLikeAuthError(jsonObject.value)) {
		...
		const loginResult = await loginCas(session, account, password, signal);
		if (!loginResult.ok) return loginResult;
		return fetchWeekEvents(session, account, weekNum, yearTerm, password, false, signal);
	}
