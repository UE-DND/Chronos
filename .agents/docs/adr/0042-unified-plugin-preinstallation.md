...
- 状态：Accepted（主题、服务端声明与在线分发见 ADR 0043 / 0044 / 0045）
- 日期：2026-09-20
...
PWA 会预缓存当前 Profile 的全部预安装资源，修订值来自最终产物摘要。URL 使用与下载器相同的完整性参数和部署 base。追加清单会保留 SvelteKit 默认的路径转换。后生成的 `_app/env.js` 与导航文档共用 `pages-cache`，随宿主更新失效。其他市场插件从固定宿主版本的 Pages 目录按需下载，不进入宿主产物或预缓存。已安装插件从数据库恢复，远程目录失败不会影响内置插件补装。分发和发布约束见 [ADR 0045](0045-versioned-online-plugin-distribution.md)。
