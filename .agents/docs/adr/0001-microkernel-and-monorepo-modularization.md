...
- **Locality（局部性）**：通用排课算法、特定高校解析与平台持久化各司其职，模块职责明确；
- **Leverage（杠杆率）**：核心包 `@chronos/core` 具备高内聚低耦合的深度接口，测试仅需纯 Node/In-process 环境即可秒级验证；
- **Extensibility（可扩展性）**：新增高校源或平台端（如 Native 客户端）只需实现对应的插件或宿主适配器，无需触动核心排课逻辑。
