...
Round 4 后主链路已深，但两处缝隙仍影响 locality 与 test leverage：

1. **`OfficialPluginService`**（~400 LOC）在单 class 内混合 catalog 拉取、SHA 校验、Dexie 持久化、ESM 运行时、ScopedContext 主题注册、CSS DOM 注入与 lifecycle FSM；无独立 seam，测试只能 mock 整棵 engine。
2. **Plugin-server proxy** wire 信封 `{ ok, payload?, error? }` 在 handler、dispatch、HTTP adapter、插件 client 五处重复声明；`dispatch.ts` 无契约单测。CQUT server 内部已深，跨 seam 传输层无 core 单源类型。
...
不变量：存储键 `core.official-plugins` / `installed_plugins`；JSON-only 主题经 `ScopedContext`（ADR 0023 §5）；`plugin-bundle.ts` 保持独立。
...
`IHttpService.proxy` 签名不变（ADR 0002）；契约作用于 HTTP body，不泛型化端口。

CQUT server 内部 `AppResult` 与 wire `PluginServerResponse` 分层不合并。
...
- **Locality**：官方安装副作用集中于 `RuntimeActivator`；proxy 信封单源
- **Depth**：facade interface 薄；四 module 可独立单测
- **Leverage**：第二个 server plugin 复用 `PluginServerResponse` 与接入清单
