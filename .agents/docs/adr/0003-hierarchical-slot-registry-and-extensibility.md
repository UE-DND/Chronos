...
1. 宿主 UI 无法知道有哪些第三方插件贡献了功能点（如新增导入源、新增底栏 Tab、扩展课表卡片徽章）；
2. 宿主页面容易写满 `switch (pluginId)` 这类按插件 id 硬编码的特殊处理；
3. 缺少结构化的插槽层级与类型安全约束。
...
- **松散耦合**：宿主 UI 只负责渲染插槽出口（`SlotOutlet`），不依赖任何具体插件的类型；
- **动态扩展**：插件在 `apply(ctx)` 中注册插槽后，宿主界面自动发现并挂载新功能，不需要修改宿主代码。
